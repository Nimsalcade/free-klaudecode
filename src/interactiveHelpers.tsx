...
import { getExternalClaudeMdIncludes, getMemoryFiles, shouldShowClaudeMdExternalIncludesWarning } from './utils/deepclimd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], deepcliInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    // Check for deepcli.md includes that need approval
    if (await shouldShowClaudeMdExternalIncludesWarning()) {
      const externalIncludes = getExternalClaudeMdIncludes(await getMemoryFiles(true));
      const {
        DeepCLIMdExternalIncludesDialog
      } = await import('./components/DeepCLIMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <DeepCLIMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
  ...
  // processes but ignored by DeepCLI itself (see auth.ts).
  ...
  // Show Chrome onboarding for first-time DeepCLI in Chrome users
  if (deepcliInChrome && !getGlobalConfig().hasCompletedDeepCLIInChromeOnboarding) {
    const {
      DeepCLIInChromeOnboarding
    } = await import('./components/DeepCLIInChromeOnboarding.js');
    await showSetupDialog(root, done => <DeepCLIInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.DEEPCLI_FRAME_TIMING_LOG;
  return {
