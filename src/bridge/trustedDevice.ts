 ...
 * (sessions_elevated_auth_enforcement in NexusAI Main); this CLI-side
 ...
 *
 * See anthropics/nexusai#274559 (spec), #310375 (B1b tenant RPCs),
...
const readStoredToken = memoize((): string | undefined => {
  ...
  const envToken = process.env.DEEPCLI_TRUSTED_DEVICE_TOKEN
  if (envToken) {
    return envToken
  }
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    // If DEEPCLI_TRUSTED_DEVICE_TOKEN is set (e.g. by an enterprise wrapper),
    // skip enrollment — the env var takes precedence in readStoredToken() so
    // any enrolled token would be shadowed and never used.
    if (process.env.DEEPCLI_TRUSTED_DEVICE_TOKEN) {
      logForDebugging(
        '[trusted-device] DEEPCLI_TRUSTED_DEVICE_TOKEN env var is set, skipping enrollment (env var takes precedence)',
      )
      return
    }
    ...
    try {
      response = await axios.post<{
        ...
        { display_name: `DeepCLI on ${hostname()} · ${process.platform}` },
        {
          headers: {
