 ...
 * (oauth-authed, same store), write to ~/.deepcli/uploads/{sessionId}/, and
 * return @path refs to prepend. DeepCLI's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches DeepCLI, just without @path.
 */
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // DEEPCLI_CUSTOM_OAUTH_URL — keep it inside the try so a bad
