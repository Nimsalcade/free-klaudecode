// Scheduled prompts, stored in <project>/.deepcli/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.deepcli', 'scheduled_tasks.json')
...
/**
 * Read and parse .deepcli/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .deepcli/scheduled_tasks.json with the given tasks. Creates .deepcli/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.deepcli'), { recursive: true })
 ...
 * written to .deepcli/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while DeepCLI
