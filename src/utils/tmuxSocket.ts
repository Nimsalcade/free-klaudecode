 ...
 * =====================
 * This module manages an isolated tmux socket for DeepCLI's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, DeepCLI could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started DeepCLI from within tmux.
 *
 * HOW IT WORKS:
 * 1. DeepCLI creates its own tmux socket: `deepcli-<PID>` (e.g., `deepcli-12345`)
 ...
 *
 * This means ANY tmux command run through DeepCLI - whether via the Tmux tool
 * directly or via Bash - will operate on DeepCLI's isolated socket, NOT the
 * user's tmux session.
 *
...
const DEEPCLI_SOCKET_PREFIX = 'deepcli'
...
/**
 * Gets the socket name for DeepCLI's isolated tmux session.
 * Format: deepcli-<PID>
 */
export function getClaudeSocketName(): string {
  if (!socketName) {
    socketName = `${DEEPCLI_SOCKET_PREFIX}-${process.pid}`
  }
  return socketName
}
...
/**
 * Gets the TMUX environment variable value for DeepCLI's isolated socket.
 *
 ...
 * the Bash tool will operate on DeepCLI's socket, NOT the user's session.
 *
 * Format: "socket_path,server_pid,pane_index" (matches tmux's TMUX env var)
 * Example: "/tmp/tmux-501/deepcli-12345,54321,0"
 *
...
/**
 * Kills the tmux server for DeepCLI's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Pass DEEPCLI_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'DEEPCLI_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set DEEPCLI_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any DeepCLI instance spawned on this socket will inherit this env var,
