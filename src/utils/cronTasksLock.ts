// Scheduler lease lock for .deepcli/scheduled_tasks.json.
//
// When multiple DeepCLI sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.deepcli', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .deepcli/ doesn't exist yet — create it and retry once. In steady
