...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.deepcli/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.deepcli/settings.json)'
    case 'projectSettings':
      return 'Project settings (.deepcli/settings.json)'
    case 'localSettings':
      return 'Local settings (.deepcli/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.deepcli/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.deepcli/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by DeepCLI)'
    default:
