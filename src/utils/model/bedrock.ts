...
> {
  ...
  try {
    ...
    // Filter for NexusAI models (SYSTEM_DEFINED filtering handled in query)
    return allProfiles
      .filter(profile => profile.inferenceProfileId?.includes('nexusai'))
...
async function createBedrockClient() {
  ...
  // Match the NexusAI Bedrock SDK's region behavior exactly:
  ...
  const skipAuth = isEnvTruthy(process.env.DEEPCLI_SKIP_BEDROCK_AUTH)
...
/**
 * Check if a model ID is a foundation model (e.g., "nexusai.deepcli-sonnet-4-5-20250929-v1:0")
 */
export function isFoundationModel(modelId: string): boolean {
  return modelId.startsWith('nexusai.')
}
 ...
 * For example:
 * - "eu.nexusai.deepcli-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.anthropic.claude-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.nexusai.deepcli-opus-4-6-v1" → "global"
 * - "anthropic.claude-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
 * - "deepcli-sonnet-4-5-20250929" → undefined (first-party format)
 */
export function getBedrockRegionPrefix(
  ...
  for (const prefix of BEDROCK_REGION_PREFIXES) {
    if (effectiveModelId.startsWith(`${prefix}.nexusai.`)) {
      return prefix
    }
  }
  return undefined
 ...
 * If the model is a foundation model (nexusai.*), the prefix will be added.
 * If the model is not a Bedrock model, it will be returned as-is.
 *
 * For example:
 * - applyBedrockRegionPrefix("us.nexusai.deepcli-sonnet-4-5-v1:0", "eu") → "eu.nexusai.deepcli-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("nexusai.deepcli-sonnet-4-5-v1:0", "eu") → "eu.nexusai.deepcli-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("deepcli-sonnet-4-5-20250929", "eu") → "deepcli-sonnet-4-5-20250929" (not a Bedrock model)
 */
export function applyBedrockRegionPrefix(
  ...
  // Check if it's a foundation model (nexusai.*) and add the prefix
