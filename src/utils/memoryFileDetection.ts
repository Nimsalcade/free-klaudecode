...
/**
 * Detects if a file path is a session-related file under ~/.deepcli.
...
/**
 * Check if a file is a DeepCLI-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: CLAUDE.md, CLAUDE.local.md, .deepcli/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (DEEPCLI_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
...
// Excludes CLAUDE.md, CLAUDE.local.md, .deepcli/rules/ (user-managed).
