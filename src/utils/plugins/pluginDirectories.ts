 ...
 * - CLI flag: --cowork
 * - Environment variable: DEEPCLI_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via DEEPCLI_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable DEEPCLI_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.DEEPCLI_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. DEEPCLI_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.deepcli/plugins or ~/.deepcli/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when DEEPCLI_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.deepcli/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.DEEPCLI_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point DEEPCLI_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.DEEPCLI_PLUGIN_SEED_DIR
  if (!raw) return []
 ...
 * ${DEEPCLI_PLUGIN_DATA}. Unlike the version-scoped install cache
 * (${DEEPCLI_PLUGIN_ROOT}, which is orphaned and GC'd on every update),
 ...
 * .replace() so this isn't invoked unless ${DEEPCLI_PLUGIN_DATA} is present
