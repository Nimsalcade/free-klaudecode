 ...
 * 1. Caching a plugin to ~/.deepcli/plugins/cache/
  ...
  if (cacheResult.path !== versionedPath) {
    ...
    if (isSubdirectory) {
      ...
      const tempPath = join(
        ...
        `.deepcli-plugin-temp-${Date.now()}-${randomBytes(4).toString('hex')}`,
      )
...
export async function installPluginFromMarketplace({
  ...
  try {
    ...
    // non-official) because dbt external_deepcli_code_plugin_installs.sql
