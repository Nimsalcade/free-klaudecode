 ...
 * Handles ${DEEPCLI_PLUGIN_ROOT}, ${user_config.X}, and general ${VAR} substitution
...
export function resolvePluginMcpEnvironment(
  ...
  switch (config.type) {
    ...
    case 'stdio': {
      ...
      // Resolve environment variables and add DEEPCLI_PLUGIN_ROOT / DEEPCLI_PLUGIN_DATA
      const resolvedEnv: Record<string, string> = {
        DEEPCLI_PLUGIN_ROOT: plugin.path,
        DEEPCLI_PLUGIN_DATA: getPluginDataDir(plugin.source),
        ...(stdioConfig.env || {}),
      }
      for (const [key, value] of Object.entries(resolvedEnv)) {
        if (key !== 'DEEPCLI_PLUGIN_ROOT' && key !== 'DEEPCLI_PLUGIN_DATA') {
    ...
    // For other types (sse-ide, ws-ide, sdk, deepcliAi-proxy), pass through unchanged
    case 'sse-ide':
    case 'ws-ide':
    case 'sdk':
