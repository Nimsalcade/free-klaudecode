 ...
 *
 * This module is responsible for discovering, loading, and validating DeepCLI plugins
 ...
 * Format: ~/.deepcli/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.deepcli/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.DEEPCLI_REMOTE)
 ...
 * DEEPCLI_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.deepcli-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".deepcli-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".deepcli-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.deepcli-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".deepcli-plugin/name")
 ...
 *
 * DEEPCLI_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 ...
 * - After modifying .deepcli-plugin/ directory (for export)
