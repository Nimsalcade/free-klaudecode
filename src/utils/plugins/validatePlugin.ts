 ...
 * one into the other. Surfaced as warnings by `deepcli plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .deepcli-plugin directory
  if (dirName === '.deepcli-plugin') {
 ...
 * '..' a user added to "climb out of .deepcli-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .deepcli-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .deepcli-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .deepcli-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `deepcli plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — DeepCLI ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. DeepCLI accepts ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.deepcli-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.deepcli-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `deepcli plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and DeepCLI ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .deepcli-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.deepcli-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .deepcli-plugin/marketplace.json or .deepcli-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
