 ...
 * When DEEPCLI_PLUGIN_USE_ZIP_CACHE is enabled and DEEPCLI_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.DEEPCLI_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires DEEPCLI_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.DEEPCLI_PLUGIN_CACHE_DIR
...
export async function getSessionPluginCachePath(): Promise<string> {
  ...
  if (!sessionPluginCachePromise) {
    sessionPluginCachePromise = (async () => {
      ...
      const dir = join(tmpdir(), `deepcli-plugin-session-${suffix}`)
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/deepcli-code#13893
  try {
