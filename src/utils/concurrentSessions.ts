...
/**
 * Kind override from env. Set by the spawner (`deepcli --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.DEEPCLI_SESSION_KIND
...
/**
 * True when this REPL is running inside a `deepcli --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `deepcli ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.DEEPCLI_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.DEEPCLI_SESSION_NAME,
              logPath: process.env.DEEPCLI_SESSION_LOG,
              agent: process.env.DEEPCLI_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `deepcli ps` sparkline
...
/**
 * Push live activity state for `deepcli ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See anthropics/deepcli-code#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.deepcli/sessions/ is shared with Windows-native DeepCLI (symlink
      // or DEEPCLI_CONFIG_DIR), a Windows PID won't be probeable from WSL
