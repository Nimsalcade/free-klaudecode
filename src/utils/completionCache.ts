...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const claudeDir = join(home, '.deepcli')
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: deepcli completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const claudeBin = process.argv[1] || 'deepcli'
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: deepcli completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes('deepcli completion') ||
...
/**
 * Regenerate cached shell completion scripts in ~/.deepcli/.
 * Called after `deepcli update` so completions stay in sync with the new binary.
 */
