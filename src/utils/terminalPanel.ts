 ...
 * socket (e.g., "deepcli-panel-a1b2c3d4") holds the shell session. Each DeepCLI
 ...
 * DeepCLI while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per DeepCLI instance (based on session ID)
...
export function getTerminalPanelSocket(): string {
  ...
  return `deepcli-panel-${sessionId.slice(0, 8)}`
}
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to DeepCLI from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to DeepCLI ', ';',
