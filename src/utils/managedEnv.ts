...
/**
 * `deepcli ssh` remote: ANTHROPIC_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.deepcli settings.env MUST NOT clobber (see
 ...
 * DEEPCLI_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.deepcli/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.DEEPCLI_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.deepcli/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.deepcli.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.deepcli/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads DEEPCLI_USE_BEDROCK,
  ...
  // when DEEPCLI_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * DEEPCLI_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
