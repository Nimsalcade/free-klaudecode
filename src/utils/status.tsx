...
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './deepclimd.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.DEEPCLI_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.DEEPCLI_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.DEEPCLI_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.DEEPCLI_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.DEEPCLI_CLIENT_KEY
      });
    }
  }
  return properties;
