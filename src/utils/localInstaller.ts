...
// hfi.tsx get a chance to set DEEPCLI_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getClaudeConfigHomeDir(), 'local')
}
export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'deepcli')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.deepcli/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'deepcli-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const wrapperPath = join(localInstallDir, 'deepcli')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/deepcli" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update DeepCLI CLI package in the local directory
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install DeepCLI CLI package: ${result.stderr}`,
      )
      logError(error)
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'deepcli'))
    return true
  } catch {
    return false
  }
