// Non-React scheduler core for .deepcli/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .deepcli/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while DeepCLI
  ...
  function check() {
    ...
    // exists to stop two DeepCLI sessions in the same cwd from double-firing
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while DeepCLI was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .deepcli/scheduled_tasks.json.\n\n` +
