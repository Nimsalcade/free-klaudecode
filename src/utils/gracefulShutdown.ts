...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // Respect DEEPCLI_DISABLE_TERMINAL_TITLE — if the user opted out of
...
function printResumeHint(): void {
  ...
  if (
    ...
    try {
      ...
      // Don't show resume hint if no session file exists (e.g., subcommands like `deepcli update`)
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (DEEPCLI_SESSIONEND_HOOKS_TIMEOUT_MS,
