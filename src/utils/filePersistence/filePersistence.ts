 ...
 * - Checks environment kind (DEEPCLI_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires DEEPCLI_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.DEEPCLI_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but DEEPCLI_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
