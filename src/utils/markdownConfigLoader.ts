...
// DeepCLI configuration directory names
export const DEEPCLI_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type ClaudeConfigDirectory = (typeof DEEPCLI_CONFIG_DIRECTORIES)[number]
 ...
 * See: https://github.com/anthropics/deepcli-code/issues/13893
 *
 ...
 * stopping there makes the parent project's `.deepcli/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.deepcli/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .deepcli directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.deepcli/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .deepcli/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.deepcli', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.deepcli', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .deepcli/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .deepcli/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .deepcli/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/deepcli-code#29599, #28182, #26992).
    //
    ...
    // This prevents the same file from appearing multiple times when ~/.deepcli is
 ...
 * 3. Can be explicitly enabled via DEEPCLI_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.deepcli/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when DEEPCLI_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.DEEPCLI_USE_NATIVE_FILE_SEARCH)
