...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - DeepCLI Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/DeepCLI/claude_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for DeepCLI Desktop config in each user directory
  ...
  throw new Error(
    'Could not find DeepCLI Desktop config file in Windows. Make sure DeepCLI Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - DeepCLI Desktop integration only works on macOS and WSL.',
    )
  }
  try {
