...
export const DEEPCLI_IN_CHROME_MCP_SERVER_NAME = 'deepcli-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[DeepCLI in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isClaudeInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === DEEPCLI_IN_CHROME_MCP_SERVER_NAME
}
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[DeepCLI in Chrome] No compatible browser found')
    return false
  }
...
export function getSocketDir(): string {
  return `/tmp/deepcli-mcp-browser-bridge-${getUsername()}`
}
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- ClaudeForChromeContext.getSocketPaths (external @ant/deepcli-for-chrome-mcp) requires a sync () => string[] callback
  ...
  const legacyName = `deepcli-mcp-browser-bridge-${getUsername()}`
...
function getSocketName(): string {
  // NOTE: This must match the one used in the DeepCLI in Chrome MCP
  return `deepcli-mcp-browser-bridge-${getUsername()}`
}
