/**
 * DeepCLI hints protocol.
 *
 * CLIs and SDKs running under DeepCLI can emit a self-closing
 * `<deepcli-code-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/deepcli-code-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type DeepCLICodeHintType = 'plugin'

export type DeepCLICodeHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: DeepCLICodeHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<deepcli-code-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractDeepCLICodeHints(
  output: string,
  command: string,
): { hints: DeepCLICodeHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<deepcli-code-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: DeepCLICodeHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[deepcliCodeHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[deepcliCodeHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[deepcliCodeHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as DeepCLICodeHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: DeepCLICodeHint | null = null
...
export function setPendingHint(hint: DeepCLICodeHint): void {
...
export function getPendingHintSnapshot(): DeepCLICodeHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetDeepCLICodeHintStore(): void {
  pendingHint = null
