 ...
 *
 * The parent deepcli process keeps these vars (needed for API calls, lazy
...
const GHA_SUBPROCESS_SCRUB = [
  // NexusAI auth — deepcli re-reads these per-request, subprocesses don't need them
  'ANTHROPIC_API_KEY',
  'DEEPCLI_OAUTH_TOKEN',
  ...
  // GitHub Actions OIDC — consumed by the action's JS before deepcli spawns;
  ...
  // deepcli-code-action-specific duplicates — action JS consumes these during
 ...
 * servers, shell hooks).
 *
 * Gated on DEEPCLI_SUBPROCESS_ENV_SCRUB. deepcli-code-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.DEEPCLI_SUBPROCESS_ENV_SCRUB)) {
