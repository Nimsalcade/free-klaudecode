 ...
 * This file provides the bridge between the external sandbox-runtime package and DeepCLI CLI's
...
/**
 * Resolve DeepCLI-specific path patterns for sandbox-runtime.
 *
 * DeepCLI uses special path prefixes in permission rules:
...
/**
 * Convert DeepCLI settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and DeepCLI temp directory as writable
  ...
  // This blocks settings in the original working directory (where DeepCLI started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.deepcli', 'settings.json'))
    denyWrite.push(resolve(cwd, '.deepcli', 'settings.local.json'))
  }

  // Block writes to .deepcli/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .deepcli/commands and
  // .deepcli/agents but not .deepcli/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full DeepCLI capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.deepcli', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.deepcli', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when DeepCLI's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/deepcli-code#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * DeepCLI's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/deepcli-code#29316.
 */
 ...
 * This is a DeepCLI CLI-specific function that updates local settings.
 */
...
/**
 * DeepCLI CLI sandbox manager - wraps sandbox-runtime with DeepCLI-specific features
 */
