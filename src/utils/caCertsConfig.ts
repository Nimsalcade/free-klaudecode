 ...
 * user-controlled files (~/.deepcli/settings.json and ~/.deepcli.json),
 ...
 *
 * We read from global config (~/.deepcli.json) and user settings
 * (~/.deepcli/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.deepcli/settings.json),
