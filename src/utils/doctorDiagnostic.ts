...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      const path = await which('deepcli')
      if (path) {
        return path
      }
    } catch {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/deepcli'))
      return join(homedir(), '.local/bin/deepcli')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.deepcli', 'local')
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/deepcli
    // Linux / macOS have prefix/bin/deepcli and prefix/lib/node_modules
    // Windows has prefix/deepcli and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'deepcli')
      : join(npmPrefix, 'bin', 'deepcli')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/deepcli
      ...
      // If no bin/deepcli exists, check for orphaned packages (no bin/deepcli symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'deepcli')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'deepcli')
    try {
...
async function detectConfigurationIssues(
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: 'Consider using native installation: deepcli install',
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: 'Run deepcli install to update configuration',
      })
    }
  }
  ...
  if (type === 'npm-local') {
    // Check if deepcli is already accessible via PATH
    const whichResult = await which('deepcli')
    const claudeInPath = !!whichResult

    // Only show warning if deepcli is NOT in PATH AND no valid alias exists
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias deepcli="~/.deepcli/local/deepcli"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias deepcli="~/.deepcli/local/deepcli"',
        })
      }
    }
  }
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: 'Do one of: (1) Re-install node without sudo, or (2) Use `deepcli install` for native installation',
      })
    }
  }
