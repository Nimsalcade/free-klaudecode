...
function getEntrypoint(): string | undefined {
  return process.env.DEEPCLI_ENTRYPOINT
}
 ...
 * This ensures tests with different DEEPCLI_CONFIG_DIR values
...
class Project {
   ...
   * DEEPCLI_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.DEEPCLI_SKIP_PROMPT_HISTORY)
    )
  }
 ...
 *
 * Streaming (deepcli.ts:~2024) emits one AssistantMessage per content_block_stop
...
/**
 * Append a periodic task summary for `deepcli ps`. Unlike ai-title this is
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.DEEPCLI_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // DEEPCLI_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.DEEPCLI_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.DEEPCLI_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
