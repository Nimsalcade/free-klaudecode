 ...
 * Protocol Handler
 *
 * Entry point for `deepcli --handle-uri <url>`. When the OS invokes deepcli
 * with a `deepcli-cli://` URL, this module:
 *   1. Parses the URI into a structured action
 *   2. Detects the user's terminal emulator
 *   3. Opens a new terminal window running deepcli with the appropriate args
 *
 ...
 * This function parses the URI, resolves the deepcli binary, and
 * launches it in the user's terminal.
 *
 * @param uri - The raw URI string (e.g., "deepcli-cli://prompt?q=hello+world")
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched DeepCLI to
...
/**
 * Handle the case where deepcli was launched as the app bundle's executable
...
/**
 * Resolve the working directory for the launched DeepCLI instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens DeepCLI.
 *
