 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches DeepCLI
...
/**
 * Launch DeepCLI in the detected terminal emulator.
 *
 ...
 * chdir(cwd) and execvp(deepcli, argv). Spaces/quotes/metacharacters in
...
async function launchWindowsTerminal(
  ...
  switch (terminal.name) {
    ...
    // mode that also keeps the session interactive after deepcli exits.
 ...
 * *child process* (deepcli.exe) uses CommandLineToArgvW, where a trailing
