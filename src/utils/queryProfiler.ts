 ...
 * pipeline from user input to first token arrival. Enable by setting DEEPCLI_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating NexusAI client
...
const ENABLED = isEnvTruthy(process.env.DEEPCLI_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set DEEPCLI_PROFILE_QUERY=1)'
  }
