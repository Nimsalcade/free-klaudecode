...
/**
 * Generates DeepCLI specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in DeepCLI's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and DeepCLI content
 ...
 *
 * This function is a critical part of DeepCLI CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows DeepCLI CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with DeepCLI defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.DEEPCLI_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - DeepCLI home: ${getClaudeConfigHomeDir()}`)
