/**
 * Perfetto Tracing for DeepCLI (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via DEEPCLI_PERFETTO_TRACE=1 or DEEPCLI_PERFETTO_TRACE=<path>
 * 2. Optionally set DEEPCLI_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run DeepCLI normally
 * 4. Trace file is written to ~/.deepcli/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.DEEPCLI_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if DEEPCLI_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.DEEPCLI_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
