...
export const getGlobalClaudeFile = memoize((): string => {
  ...
  const filename = `.deepcli${fileSuffixForOauthConfig()}.json`
  return join(process.env.DEEPCLI_CONFIG_DIR || homedir(), filename)
})
 ...
 * If DEEPCLI_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.DEEPCLI_HOST_PLATFORM
