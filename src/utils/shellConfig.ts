 ...
 * Used for managing deepcli aliases and PATH entries
 */
...
export const DEEPCLI_ALIAS_REGEX = /^\s*alias\s+deepcli\s*=/
...
/**
 * Filter out installer-created deepcli aliases from an array of lines
 * Only removes aliases pointing to $HOME/.deepcli/local/deepcli
...
export function filterClaudeAliases(lines: string[]): {
  ...
  const filtered = lines.filter(line => {
    // Check if this is a deepcli alias
    if (DEEPCLI_ALIAS_REGEX.test(line)) {
      // Extract the alias target - handle spaces, quotes, and various formats
      // First try with quotes
      let match = line.match(/alias\s+deepcli\s*=\s*["']([^"']+)["']/)
      if (!match) {
        // Try without quotes (capturing until end of line or comment)
        match = line.match(/alias\s+deepcli\s*=\s*([^#\n]+)/)
      }
...
/**
 * Check if a deepcli alias exists in any shell config file
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (DEEPCLI_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+deepcli=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a deepcli alias exists and points to a valid executable
