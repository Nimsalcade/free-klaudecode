...
import {
  DEEPCLI_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
...
/**
 * If filePath is inside a .deepcli/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .deepcli/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.deepcli', 'skills')),
      prefix: '/.deepcli/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.deepcli', 'skills')),
      prefix: '~/.deepcli/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.deepcli/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.deepcli/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.deepcli${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.deepcli${sep}settings.local.json`)
  ) {
    // Include .deepcli/settings.json even for other projects
    return true
  }
...
// Always ask when DeepCLI tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .deepcli/commands or .deepcli/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.deepcli', 'commands')
  const agentsDir = join(getOriginalCwd(), '.deepcli', 'agents')
  const skillsDir = join(getOriginalCwd(), '.deepcli', 'skills')
 ...
 * Path format: ~/.deepcli/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for DeepCLI to write temporary files.
...
/**
 * Returns the user-specific DeepCLI temp directory name.
 * On Unix: 'deepcli-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'deepcli' (tmpdir() is already per-user)
 */
...
/**
 * Returns the DeepCLI temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/deepcli-{uid}/ (resolved to /private/tmp/deepcli-{uid}/ on macOS)
 * - On Windows: {tmpdir}/deepcli/ (e.g., C:\Users\{user}\AppData\Local\Temp\deepcli\)
 * This is a per-user temporary directory used by DeepCLI for all temp files.
 *
 ...
 * resolution, paths like /tmp/deepcli-{uid}/... wouldn't match /private/tmp/deepcli-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (DEEPCLI_TMPDIR env + platform) are
 ...
 * Path format: /tmp/deepcli-{uid}/{sanitized-cwd}/
 */
 ...
 * Path format: /tmp/deepcli-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/deepcli-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .deepcli/worktrees/ is a structural path (where DeepCLI stores
      // git worktrees), not a user-created dangerous directory. Skip the .deepcli
      // segment when it's followed by 'worktrees'. Any nested .deepcli directories
      ...
      if (dir === '.deepcli') {
        ...
        if (
          ...
          break // Skip this .deepcli, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .deepcli , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .deepcli , .bashrc..., settings.json.
 ...
 * - DeepCLI config files (.deepcli/settings.json, .deepcli/commands/, .deepcli/agents/)
 * - MCP CLI state files (managed internally by DeepCLI)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `DeepCLI requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for DeepCLI config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `DeepCLI requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `DeepCLI requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .deepcli/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `DeepCLI requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `DeepCLI requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `DeepCLI requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `DeepCLI requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .deepcli is a dangerous directory
  ...
  // 1.6. Check for .deepcli/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .deepcli/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .deepcli/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.deepcli) rule in userSettings (e.g. from sandbox
  ...
  // "allow DeepCLI to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .deepcli/ (project or global).
    // Accepts both the broad patterns ('/.deepcli/**', '~/.deepcli/**') and
    // narrowed ones like '/.deepcli/skills/my-skill/**' so users can grant
    ...
    // '/.deepcli/../**' from leaking this bypass outside .deepcli/.
    const ruleContent = claudeFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      (ruleContent.startsWith(DEEPCLI_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, DeepCLI config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .deepcli/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.deepcli/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.deepcli/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    const jobDir = process.env.DEEPCLI_JOB_DIR
    if (jobDir) {
      ...
      // the case where ~/.deepcli is a symlink (e.g. to /data/deepcli-config).
  ...
  // ~/.deepcli/, which is in DANGEROUS_DIRECTORIES. The DEEPCLI_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .deepcli/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs DeepCLI to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .deepcli/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .deepcli/ only (not ~/.deepcli/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.deepcli', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/deepcli/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.deepcli/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.deepcli/teams/) for swarm coordination
