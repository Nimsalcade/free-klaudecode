...
import { accumulateUsage, updateUsage } from '../services/api/deepcli.js'
 ...
 * to share the parent's prompt cache. The NexusAI API cache key is composed of:
 ...
 * deepcli.ts (but only for older models that do not use adaptive thinking).
...
export type ForkedAgentParams = {
   ...
   * AND budget_tokens (via clamping in deepcli.ts). If the fork uses cacheSafeParams
...
export async function runForkedAgent({
  ...
  // tool_uses are repaired downstream by ensureToolResultPairing in deepcli.ts,
