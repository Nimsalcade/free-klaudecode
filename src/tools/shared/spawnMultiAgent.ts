 ...
 * When outside tmux: Creates a deepcli-swarm session with all teammates in a tiled layout.
 */
async function handleSpawnSplitPane(
  ...
  // - Outside both: creates deepcli-swarm session with tiled teammates
  ...
  // Build the command to spawn DeepCLI with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces DEEPCLI_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, DEEPCLI_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new DeepCLI instance.
