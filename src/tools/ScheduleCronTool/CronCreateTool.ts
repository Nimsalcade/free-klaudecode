...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .deepcli/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this DeepCLI session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .deepcli/scheduled_tasks.json'
      : 'Session-only (not written to disk, dies when DeepCLI exits)'
    return {
