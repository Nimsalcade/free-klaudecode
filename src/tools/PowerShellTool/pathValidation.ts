...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .deepcli is a dangerous directory
  // and internal editable paths live under ~/.deepcli/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/deepcli/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.deepcli; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.deepcli/settings.json (DeepCLI's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.deepcli`, validate subsequent statements with
  // cwd='./.deepcli'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, DeepCLI may only access files in the allowed working directories for this session: ${dirListStr}.`
