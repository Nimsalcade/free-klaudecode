 ...
 * These are dynamically imported only when `deepcli plugin *` or `deepcli plugin marketplace *` runs.
 */
...
export async function pluginValidateHandler(
  ...
  try {
    ...
    // If this is a plugin manifest located inside a .deepcli-plugin directory,
    ...
    if (result.fileType === 'plugin') {
      ...
      if (basename(manifestDir) === '.deepcli-plugin') {
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `deepcli plugin install` to install a plugin.',
      )
    }
  }
