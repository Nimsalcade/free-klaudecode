 ...
 * - Epoch management: reads worker_epoch from DEEPCLI_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (DEEPCLI_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * DEEPCLI_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    DEEPCLI_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.DEEPCLI_WORKER_EPOCH
