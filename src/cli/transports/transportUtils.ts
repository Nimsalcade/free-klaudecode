 ...
 * 1. SSETransport (SSE reads + POST writes) when DEEPCLI_USE_CCR_V2 is set
 * 2. HybridTransport (WS reads + POST writes) when DEEPCLI_POST_FOR_SESSION_INGRESS_V2 is set
...
export function getTransportForUrl(
  ...
  if (url.protocol === 'ws:' || url.protocol === 'wss:') {
    if (isEnvTruthy(process.env.DEEPCLI_POST_FOR_SESSION_INGRESS_V2)) {
