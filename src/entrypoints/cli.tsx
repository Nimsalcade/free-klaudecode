...
if (typeof MACRO === 'undefined') {
  (globalThis as any).MACRO = {
    ...
    PACKAGE_URL: 'deepcli-code-source-snapshot',
...
if (process.env.DEEPCLI_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.DEEPCLI_ABLATION_BASELINE) {
  for (const k of ['DEEPCLI_SIMPLE', 'DEEPCLI_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'DEEPCLI_DISABLE_AUTO_MEMORY', 'DEEPCLI_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (DeepCLI)`);
    return;
  }
  ...
  if (process.argv[2] === '--deepcli-in-chrome-mcp') {
    profileCheckpoint('cli_claude_in_chrome_mcp_path');
    const {
      runClaudeInChromeMcpServer
    } = await import('../utils/deepcliInChrome/mcpServer.js');
    ...
    const {
      runChromeNativeHost
    } = await import('../utils/deepcliInChrome/chromeNativeHost.js');
  ...
  // Fast-path for `deepcli remote-control` (also accepts legacy `deepcli remote` / `deepcli sync` / `deepcli bridge`):
  ...
  // Fast-path for `deepcli daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `deepcli ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.deepcli/sessions/ registry. Flag
  ...
  // Fast-path for `deepcli environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `deepcli self-hosted-runner`: headless self-hosted-runner
