 ...
 * reply and emits notifications/deepcli/channel/permission with
 ...
 * capabilities.experimental['deepcli/channel/permission'].
 *
 * Kenneth's "would this let DeepCLI self-approve?": the approving party is
 * the human via the channel, not DeepCLI. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/deepcli/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/deepcli/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['deepcli/channel'] !== undefined &&
      c.capabilities?.experimental?.['deepcli/channel/permission'] !== undefined,
  )
}
 ...
 * (notifications/deepcli/channel/permission) with the structured payload.
