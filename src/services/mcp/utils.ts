...
export function getProjectMcpServerStatus(
  ...
  // flagSettings/policySettings but NOT projectSettings (repo-level .deepcli/settings.json).
  ...
  // In non-interactive mode (SDK, deepcli -p, piped input), there's no way to
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, deepcliAi-proxy, sse-ide, ws-ide)
