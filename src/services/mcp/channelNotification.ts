 ...
 *   - sends `notifications/deepcli/channel` notifications for inbound — this file
 *
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/deepcli/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['deepcli/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/deepcli/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/deepcli/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/deepcli/channel/permission_request'
 ...
 *             not registered.
 *   register  Subscribe to notifications/deepcli/channel.
 *
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['deepcli/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/deepcli/channel).
  if (!capabilities?.experimental?.['deepcli/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare deepcli/channel capability',
    }
  }
  ...
  if (entry.kind === 'plugin') {
    // Marketplace verification: the tag is intent (plugin:slack@nexusai),
    // the runtime name is just plugin:slack:X — could be slack@nexusai or
