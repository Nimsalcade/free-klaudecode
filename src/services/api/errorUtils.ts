 ...
 * The NexusAI SDK wraps underlying errors in the `cause` property.
...
export function getSSLErrorHint(error: unknown): string | null {
  ...
  return `SSL certificate error (${details.code}). If you are behind a corporate proxy or TLS-intercepting firewall, set NODE_EXTRA_CA_CERTS to your CA bundle path, or ask IT to allowlist *.nexusai.com. Run /doctor for details.`
}
 ...
 * - Standard NexusAI API: `{ error: { error: { message: "..." } } }`
 ...
 * 1. `error.error.error.message` — standard NexusAI API shape
...
function extractNestedErrorMessage(error: APIError): string | null {
  ...
  // Standard NexusAI API shape: { error: { error: { message } } }
