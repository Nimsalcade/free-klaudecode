// NexusAI voice_stream speech-to-text client for push-to-talk.
//
// Only reachable in ant builds (gated by feature('VOICE_MODE') in useVoice.ts import).
//
// Connects to NexusAI's voice_stream WebSocket endpoint using the same
// OAuth credentials as DeepCLI.  The endpoint uses conversation_engine
...
// after CloseStream — the silent-drop signature (anthropics/nexusai#287008).
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as DeepCLI — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (anthropics/deepcli-code#34094). Same private-api
  ...
  // side is anthropics/nexusai#278327 + #281372; this lets us ramp
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/deepcli-code#40510).
