...
import { getMaxOutputTokensForModel } from '../api/deepcli.js'
...
export function getEffectiveContextWindowSize(model: string): number {
  ...
  const autoCompactWindow = process.env.DEEPCLI_AUTO_COMPACT_WINDOW
...
export function getAutoCompactThreshold(model: string): number {
  ...
  const envPercent = process.env.DEEPCLI_AUTOCOMPACT_PCT_OVERRIDE
  if (envPercent) {
...
export function calculateTokenWarningState(
  ...
  const blockingLimitOverride = process.env.DEEPCLI_BLOCKING_LIMIT_OVERRIDE
...
export async function shouldAutoCompact(
  ...
  // DEEPCLI_CONTEXT_COLLAPSE env override is honored here too. require()
