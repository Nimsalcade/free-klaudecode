...
// Skills can be large (verify=18.7KB, deepcli-api=20.1KB). Previously re-injected
  ...
  try {
    if (promptCacheSharingEnabled) {
      try {
        ...
        // budget_tokens via Math.min(budget, maxOutputTokens-1) in deepcli.ts,
...
function shouldExcludeFromPostCompactRestore(
  ...
  // Exclude all types of deepcli.md files
  // TODO: Refactor to use isMemoryFilePath() from deepclimd.ts for consistency
  // and to also match child directory memory files (.deepcli/rules/*.md, etc.)
  try {
