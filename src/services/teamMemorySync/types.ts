 ...
 * Based on the backend API contract from nexusai/nexusai#250711.
 */
...
export const TeamMemoryContentSchema = lazySchema(() =>
  z.object({
    ...
    // nexusai/nexusai#283027. Optional for forward-compat with older
...
/**
 * Full response from GET /api/deepcli_code/team_memory
 */
...
/**
 * Structured 413 error body from the server (nexusai/nexusai#293258).
...
export type TeamMemorySyncUploadResult = {
  ...
  httpStatus?: number
  /**
   * Structured error_code from a parsed 413 body (nexusai/nexusai#293258).
