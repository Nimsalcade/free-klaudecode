 ...
 *
 * API contract (nexusai/nexusai#250711 + #283027):
 *   GET  /api/deepcli_code/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/deepcli_code/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/deepcli_code/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
// Per-entry size cap — server default from nexusai/nexusai#293258.
...
// GB-tunable per-org (deepcli_code_team_memory_limits), so any compile-time
...
export type SyncState = {
   ...
   * (nexusai/nexusai#293258 adds error_code + extra_details.max_entries).
 ...
 * Format matches the server's entryChecksums values (nexusai/nexusai#283027)
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken &&
      tokens.scopes?.includes(DEEPCLI_AI_INFERENCE_SCOPE) &&
      tokens.scopes.includes(DEEPCLI_AI_PROFILE_SCOPE),
  )
}
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/deepcli_code/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
 ...
 * Requires nexusai/nexusai#283027 deployed; on failure the caller fails the
...
async function fetchTeamMemoryHashes(
  ...
  try {
    ...
    // Requires nexusai/nexusai#283027. If entryChecksums is missing,
...
async function uploadTeamMemory(
  ...
  try {
    ...
    // Parse structured 413 (nexusai/nexusai#293258). The server's
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // 413's extra_details.max_entries — nexusai/nexusai#293258).  The
  // server's entry-count cap is GB-tunable per-org via
  // deepcli_code_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  // Requires nexusai/nexusai#283027 — if the response lacks entryChecksums
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/deepclimd.js')
...
export async function pushTeamMemory(
  ...
  for (
    ...
    if (!result.conflict) {
      ...
      // max_entries (nexusai/nexusai#293258), cache it so the next push
    ...
    if (!probe.success || !probe.entryChecksums) {
      // Requires nexusai/nexusai#283027. A transient probe failure here is
