// OAuth client for handling authentication flows with DeepCLI services
...
export function shouldUseClaudeAIAuth(scopes: string[] | undefined): boolean {
  return Boolean(scopes?.includes(DEEPCLI_AI_INFERENCE_SCOPE))
}
...
export function buildAuthUrl({
  ...
  const authUrlBase = loginWithClaudeAi
    ? getOauthConfig().DEEPCLI_AI_AUTHORIZE_URL
    : getOauthConfig().CONSOLE_AUTHORIZE_URL

  const authUrl = new URL(authUrlBase)
  authUrl.searchParams.append('code', 'true') // this tells the login page to show DeepCLI Max upsell
  ...
  const scopesToUse = inferenceOnly
    ? [DEEPCLI_AI_INFERENCE_SCOPE] // Long-lived inference-only tokens
    : ALL_OAUTH_SCOPES
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full DeepCLI AI set. The
  ...
  try {
    ...
    // DEEPCLI_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.DEEPCLI_ACCOUNT_UUID
  const envUserEmail = process.env.DEEPCLI_USER_EMAIL
  const envOrganizationUuid = process.env.DEEPCLI_ORGANIZATION_UUID
