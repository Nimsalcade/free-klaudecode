...
export function getLocalISODate(): string {
  ...
  if (process.env.DEEPCLI_OVERRIDE_DATE) {
    return process.env.DEEPCLI_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.DEEPCLI_OVERRIDE_DATE
    ? new Date(process.env.DEEPCLI_OVERRIDE_DATE)
    : new Date()
