...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets DeepCLI handle permission prompts automatically — DeepCLI checks each tool call for risky actions and prompt injection before executing. Actions DeepCLI identifies as safe are executed, while actions DeepCLI identifies as risky are blocked and DeepCLI may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. DeepCLI can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
...
export function AutoModeOptInDialog(t0) {
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}><Text>{AUTO_MODE_DESCRIPTION}</Text><Link url="https://docs.deepcli.dev/docs/en/security" /></Box>;
    $[4] = t3;
  } else {
    t3 = $[4];
  }
