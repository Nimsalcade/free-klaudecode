...
import { useClaudeAiLimits } from '../../services/deepcliAiLimitsHook.js';
...
export function Notifications(t0) {
  ...
  const isInOverageMode = deepcliAiLimits.isUsingOverage;
  let t7;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.DEEPCLI_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
