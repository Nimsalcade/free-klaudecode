...
> {
  ...
  // the turn returns — otherwise `deepcli list` shows stale state for the gap.
  ...
  if (
    ...
    const p = jobClassifierModule!
      .classifyAndWriteState(process.env.DEEPCLI_JOB_DIR, turnAssistantMessages)
      .catch(err => {
  ...
  if (!isBareMode()) {
    ...
    if (!isEnvDefinedFalsy(process.env.DEEPCLI_ENABLE_PROMPT_SUGGESTION)) {
