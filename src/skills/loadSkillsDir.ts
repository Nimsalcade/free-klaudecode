...
/**
 * Returns a deepcli config directory path for a given source.
 */
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.deepcli', dir)
    case 'userSettings':
...
export function createSkillCommand({
  ...
  return {
    ...
    async getPromptForCommand(args, toolUseContext) {
      ...
      // Replace ${DEEPCLI_SKILL_DIR} with the skill's own directory so bash
      ...
      if (baseDir) {
        ...
        finalContent = finalContent.replace(/\$\{DEEPCLI_SKILL_DIR\}/g, skillDir)
      }

      // Replace ${DEEPCLI_SESSION_ID} with the current session ID
      finalContent = finalContent.replace(
        /\$\{DEEPCLI_SESSION_ID\}/g,
        getSessionId(),
      )

      // Security: MCP skills are remote and untrusted — never execute inline
      // shell commands (!`…` / ```! … ```) from their markdown body.
      // ${DEEPCLI_SKILL_DIR} is meaningless for MCP skills anyway.
      if (loadedFrom !== 'mcp') {
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.deepcli', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.DEEPCLI_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.deepcli', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.deepcli/skills from
