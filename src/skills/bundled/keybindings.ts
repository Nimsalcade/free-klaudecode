...
const FILE_FORMAT_EXAMPLE: KeybindingsSchemaType = {
  $schema: 'https://www.schemastore.org/deepcli-code-keybindings.json',
  $docs: 'https://docs.deepcli.dev/docs/en/keybindings',
  bindings: [
    {
      context: 'Chat',
      bindings: {
...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.deepcli/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.deepcli/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.deepcli/keybindings.json`.',
  '',
  ...
  'Location: ~/.deepcli/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.deepcli/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
