...
/**
 * Surfaces plugin-install prompts driven by `<deepcli-code-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/deepcli-code-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/deepcliCodeHints.js';
...
type UseDeepCLICodeHintRecommendationResult = {
  recommendation: PluginHintRecommendation | null;
  handleResponse: (response: 'yes' | 'no' | 'disable') => void;
};
export function useDeepCLICodeHintRecommendation() {
  const $ = _c(11);
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useDeepCLICodeHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
