// React hook for hold-to-talk voice input using NexusAI voice_stream STT.
//
...
// audio module (macOS) or SoX for recording, and NexusAI's voice_stream
...
export function useVoice({
  ...
  // transcripts (anthropics/nexusai#287008 session-sticky variant); when
      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that DeepCLI has microphone access.',
            )
          } else {
  ...
  async function startRecordingSession(): Promise<void> {
    ...
    // teardown window (anthropics/nexusai#287008 surfaces this as
