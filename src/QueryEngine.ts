...
import { accumulateUsage, updateUsage } from 'src/services/api/deepcli.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // DEEPCLI_COWORK_MEMORY_PATH_OVERRIDE, inject the memory-mechanics prompt.
    // The env var is an explicit opt-in signal — the caller has wired up
    // a memory directory and needs DeepCLI to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.DEEPCLI_EAGER_FLUSH) ||
          isEnvTruthy(process.env.DEEPCLI_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via DEEPCLI_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or DEEPCLI_PLUGIN_SEED_DIR before this runs;
    ...
    for await (const message of query({
      ...
      if (
        ...
        // If the SDK subprocess restarts before then (deepcli-desktop kills
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. deepcli.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see deepcli.ts message_delta
...
/**
 * Sends a single prompt to the DeepCLI API and returns the response.
 * Assumes that deepcli is being used non-interactively -- will not
