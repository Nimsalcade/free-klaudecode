...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to DeepCLI.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
        '  deepcli mcp add --transport http sentry https://mcp.sentry.dev/mcp\n\n' +
        '  # Add HTTP server with headers:\n' +
        '  deepcli mcp add --transport http corridor https://app.corridor.dev/api/mcp --header "Authorization: Bearer ..."\n\n' +
        '  # Add stdio server with environment variables:\n' +
        '  deepcli mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  deepcli mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
    ...
    .addOption(
      new Option(
        '--xaa',
        "Enable XAA (SEP-990) for this server. Requires 'deepcli mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).",
    ...
    .action(async (name, commandOrUrl, args, options) => {
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            'Usage: deepcli mcp add <name> <command> [args...]',
        )
      ...
      try {
        ...
        if (options.xaa && !isXaaEnabled()) {
          cliError(
            'Error: --xaa requires DEEPCLI_ENABLE_XAA=1 in your environment',
          )
        }
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              "'deepcli mcp xaa setup' (settings.xaaIdp not configured)",
            )
          }
          if (missing.length) {
        ...
        if (transport === 'sse') {
          ...
          if (!transportExplicit && looksLikeUrl) {
            ...
            process.stderr.write(
              `If this is an HTTP server, use: deepcli mcp add --transport http ${name} ${actualCommand}\n`,
            )
            process.stderr.write(
              `If this is an SSE server, use: deepcli mcp add --transport sse ${name} ${actualCommand}\n`,
            )
          }
