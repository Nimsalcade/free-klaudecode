...
function RateLimitOptionsMenu(t0) {
  ...
  bb0: {
    let actionOptions;
    if ($[2] !== deepcliAiLimits.overageDisabledReason || $[3] !== deepcliAiLimits.overageStatus) {
      actionOptions = [];
      if (extraUsage.isEnabled()) {
        const hasBillingAccess = hasClaudeAiBillingAccess();
        const needsToRequestFromAdmin = isTeamOrEnterprise && !hasBillingAccess;
        const isOrgSpendCapDepleted = deepcliAiLimits.overageDisabledReason === "out_of_credits" || deepcliAiLimits.overageDisabledReason === "org_level_disabled_until" || deepcliAiLimits.overageDisabledReason === "org_service_zero_credit_limit";
        if (needsToRequestFromAdmin && isOrgSpendCapDepleted) {} else {
          const isOverageState = deepcliAiLimits.overageStatus === "rejected" || deepcliAiLimits.overageStatus === "allowed_warning";
          let label;
      ...
      $[2] = deepcliAiLimits.overageDisabledReason;
      $[3] = deepcliAiLimits.overageStatus;
      $[4] = actionOptions;
    } else {
      actionOptions = $[4];
    }
